...
| 2 | PR merge gate (Copilot wait + revise) — after `gh pr create` (or auto-merge flip), poll for a Copilot review, walk every Copilot thread (`validate → fix → reply → resolve`, re-trigger via `gh pr edit <n> --add-reviewer @copilot` after each fix push since Copilot does not re-review on push alone). Merge only when (a) DEE-114 `Copilot review` required status check is `success`, (b) all Copilot threads (incl. "Potential fix" auto-pushes) are resolved (INVALID/DEFER per ack / 24 h SLA / linked DEFER issue), and (c) no new Copilot comment, review, or auto-push commit in the last 10 min (full quiet window). **Termination:** convergence (zero new threads) OR 5-round cap with stylistic-only leftovers DEFERred to a follow-up issue OR meta-recursion (Copilot bikeshedding the SOP itself). **Review state is NOT checked** — `copilot-pull-request-reviewer[bot]` only ever emits `COMMENTED` (DEE-115 dogfood + DEE-114 confirmation across PRs #18–#33). No carve-out for closer or artifact PRs. | `_bmad-output/project-context.md` §15 ("PR merge gate (Copilot wait + revise)") — including step 7 termination + §19 closer-PR carve-out clarification | Wes (Workflow Builder) | DEE-115 (child of DEE-113 CEO policy) |
