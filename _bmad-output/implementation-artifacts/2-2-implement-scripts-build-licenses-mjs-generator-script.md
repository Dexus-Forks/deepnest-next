...
- `tests/assets/.fixture-manifest.json` — **necessary scope-add (round-2 / CI-driven).** First push surfaced the exact FR-02/FR-03 coupling that AC-02.2.9 warned about: Story 3.1's `test:fixtures:check` walks every file under `tests/assets/` and the `LICENSE.yml` header reduction + 2-row `copyright:` trim changed its size (2016→1340 bytes) + sha256, tripping the manifest drift gate. Resolution path is the one Story 3.1 explicitly supports for deliberate fixture edits: `npm run test:fixtures:update && git add tests/assets/.fixture-manifest.json` (per `scripts/check-test-fixtures.mjs:255` docs pointer). Updated manifest commits the new size/sha256 for `tests/assets/LICENSE.yml`. The two SVG specimens are unchanged. AC-02.2.9 (β with deferred impl) recommendation now has a freshly-validated empirical case for the Story 3.1.x narrowing.
...
**Untouched (verified by `git diff`):** `scripts/check-test-fixtures.mjs`, `package-lock.json`, `eslint.config.mjs`, `tsconfig.json`, `.github/`, all `main/util/*.{js,ts}`, all `main/util/_unused/*` source files, all `main/font/fonts/*`, all `tests/assets/*.svg` / `*.woff` / `*.woff2`, `main.js`, `index.d.ts`. (Note: `tests/assets/.fixture-manifest.json` was originally guarded as "untouched" per AC-02.2.13's intent; round-2 added it as a necessary scope-add — see Modified files row above.)
...
| 2026-04-26 | Round-1 Board Review APPROVED `severity.max=P2` ([DEE-93](/DEE/issues/DEE-93) / Sage). 0×P0 + 0×P1 + 4×P2 + 6×P3 across 4 dispatched perspectives; all closure-recommended into Story 2.3 (A4) scope-adds + a single ADR-008 amendment PR. Story 2.1 Round-1 follow-ups (P2-01 + P3-01 + P3-02 + P3-03) verified closed in this PR. | Sage (Review Leader, BMad) |
| 2026-04-26 | Round-2 fixup: first PR push tripped Story 3.1's `test:fixtures:check` gate on the `tests/assets/LICENSE.yml` header reduction (size 2016→1340 bytes). FR-02/FR-03 coupling exactly per AC-02.2.9 warning. Resolution: `npm run test:fixtures:update` then commit the regenerated `tests/assets/.fixture-manifest.json`. Necessary scope-add disclosed in File List above. | Amelia (Dev, BMad) |
