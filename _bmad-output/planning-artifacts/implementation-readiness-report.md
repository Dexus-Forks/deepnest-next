...
| **Schema** | `{ "captured_at": "<iso8601>", "runner_cell": "ubuntu-22.04 / node-22.x", "git_sha": "<sha>", "runs": [<5 wall-clock-ms numbers>], "rolling_mean_ms": <number>, "stddev_ms": <number>, "tolerance_pct": 20 }` |
| **Procedure** | (1) Add Playwright JSON reporter to `playwright.config.ts`, guarded for CI-only. (2) Run the canonical CI cell 5×, same SHA. (3) Extract per-run wall-clock duration. (4) Commit the 5 numbers + rolling mean + stddev. (5) Optionally amend `prd.md` §NFR-01 to reference the file as the authoritative baseline. |
