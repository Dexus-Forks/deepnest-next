...
> **Note on the verdict literal.** The verdict token remains `APPROVED` (the same 2-state model as the default rule); `(via carve-out)` is an annotation noting the §2 invocation path, NOT a third verdict state. Sage's verdict-generation, audit-log entries, and downstream tooling continue to treat the outcome as `APPROVED`; the annotation merely flags that the invocation went through the 5+2 gate.
...
> **Bridging note — sign-off triplet vs vote pattern.** This vote pattern is distinct from the §2.1 (4) sign-off triplet — the sign-offs vet the **technical claim** that the production hole is closed (CTO + TEA + Aegis = three independent technical perspectives); the board vote consents to **invoking the carve-out** on the verdict thread (CTO + PM + Sage = governance + scope + verdict-author voice). CTO appears in both because the governance read and the technical read are both binding for invocation; TEA + Aegis are technical-only; PM + Sage are governance / verdict-only. See §10 for one-line role definitions of each perspective.
...
- **Finding F1** (Lydia, P1): regression-test gap on `scripts/check-licenses-budget.mjs:62-68` — the spawn-failure guard (`r.error || r.signal`) lacked a unit test. **Mechanism:** without the guard, `r.status` is `null` on spawn-failure / signal-exit paths and `process.exit(null)` resolves to exit 0, which would falsely pass the budget gate; with the guard but no regression test, future refactors could re-open the hole undetected.
...
## 10. Perspective roster

The reviewer perspectives referenced across this SOP and the [DEE-131](/DEE/issues/DEE-131) lived precedent thread, with one-line role definitions. New perspectives appearing on a future invocation should be appended here by amendment, not invoked ad hoc.

| Perspective | Role / agent                                                                                                       | Where referenced in this SOP                                                                                                                                                                                                          |
| ----------- | ------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Aegis**   | Security Reviewer — defensive-review perspective; sign-off triplet member in §2.1 (4). Agent `859aa171-f01c-4ed8-bb5e-19f3e2113ad3`. | §2.1 (4) sign-off triplet; [DEE-131](/DEE/issues/DEE-131) board minion; [DEE-136](/DEE/issues/DEE-136) follow-up.                                                                                                                     |
| **Lydia**   | Code-Quality Reviewer — readability / regression-coverage perspective; F1 author in the lived precedent.            | §4.1 F1 author; [DEE-131](/DEE/issues/DEE-131) board minion.                                                                                                                                                                          |
| **Vitra**   | Architecture Reviewer — coupling / boundary perspective.                                                           | Story 2.1 / 2.2 / 2.3 board minions (referenced indirectly via [DEE-131](/DEE/issues/DEE-131) thread).                                                                                                                                |
| **Hermes**  | Performance Reviewer — runtime / resource perspective.                                                              | Story 1.1 / 2.1 / 2.2 / 2.3 board minions (referenced indirectly via [DEE-131](/DEE/issues/DEE-131) thread).                                                                                                                          |
| **Murat (TEA)** | Test-Engineer Architect — closer-PR + trace-matrix author; sign-off triplet member in §2.1 (4).                  | §2.1 (4) sign-off triplet; §4.2 [comment f09bf227](/DEE/issues/DEE-131#comment-f09bf227-7bef-462d-bbe8-bb94d6084721) (criterion-5 closure evidence).                                                                                   |
| **CTO**     | Governance / Cloud-Dragonborn voice — sign-off triplet member in §2.1 (4); vote-pattern member in §2.4.            | §2.1 (4) sign-off triplet; §2.4 vote pattern; §4.2 arbitration ([comment a44fac6a](/DEE/issues/DEE-131#comment-a44fac6a-8e42-42ab-bb32-928131dfd9d3)) + close-out ([comment c2814303](/DEE/issues/DEE-131#comment-c2814303-630a-49d9-b72e-97cfdd6e13fc)). |
| **Sage**    | Review Leader / verdict-author voice — vote-pattern member in §2.4; verdict author per [DEE-131](/DEE/issues/DEE-131) precedent. | §2.4 vote pattern; §4.2 [comment f119dae2](/DEE/issues/DEE-131#comment-f119dae2-78d7-4e48-8149-acbab6473152); §5 step 1 verdict-comment author.                                                                                       |
| **PM (John)** | Product Manager — spec-author voice; vote-pattern member in §2.4.                                                | §2.4 vote pattern; §4.2 [comment bb3c7eb2](/DEE/issues/DEE-131#comment-bb3c7eb2-1fb3-4304-ae4a-a4d70ddbd71f) (5-criterion gate articulation).                                                                                          |
| **Amelia**  | Dev (DS) — implementation perspective; verifies criterion 5 wire-up via API in the lived precedent.                | §4.2 [comment eabbbd3c](/DEE/issues/DEE-131#comment-eabbbd3c-17ea-472f-ad40-3f0c06490162) (criterion 5 + invariant 6 evidence).                                                                                                       |

The triplet (CTO + TEA + Aegis) in §2.1 (4) is the **technical sign-off stack**; the triplet (CTO + PM + Sage) in §2.4 is the **board-vote stack** (see §2.4 bridging note). CTO is the only perspective that appears in both — governance read + technical read are both binding for invocation.

---

_Last updated: 2026-04-27 (John, [DEE-145](/DEE/issues/DEE-145) — spec-clarity follow-up bundle: §2.2 verdict-state-vs-annotation clarifying note; §2.4 sign-off-triplet vs vote-pattern bridging note; §4.1 F1 `process.exit(null)` mechanism narrative + `:62-68` line range; §10 perspective-roster appendix. Origin: [DEE-142](/DEE/issues/DEE-142) PR #40 sign-off thread — CTO obs 1 + 2 + Sage obs 3 + 4 + post-merge `f9d3413` clarifying-narrative delta. Pending CTO + Sage sign-off on PR. Prior: 2026-04-27 (John, [DEE-142](/DEE/issues/DEE-142) authoring — initial draft against scope-lock 5 + 2 per CTO close-out [c2814303](/DEE/issues/DEE-131#comment-c2814303-630a-49d9-b72e-97cfdd6e13fc); merged at [c6550f9](https://github.com/Dexus-Forks/deepnest-next/commit/c6550f9c9baa1f4051d3988b2bd5a7ace5c47975) / [PR #40](https://github.com/Dexus-Forks/deepnest-next/pull/40)).)._
